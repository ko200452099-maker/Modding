...
> **Purpose:** Single source of truth before we plan the Next Update.  
> **Date:** 2026-09-10 · Africa/Tripoli  
> **Inputs:** `ModLoader.csa` 480,326 B / 29,938 lines / CRLF, `ModLoader_Statics.c` (27 tunables), disassembly via `tools/csa_decompile.py` (283 Functions, 2,466 Labels, 311 natives, 1,803 strings) + 87 loadable scripts.  
> **Status:** Step 1 — *Understand* (next: Step 2 Roadmap → `docs/ROADMAP_vNext.md`)
...
Boot guard (GET_THIS_SCRIPT_NAME hash) → Label_0 init (safe-zone, colors, Static[0..128])
  → Label_4: while(true) { Label_3: WAIT(0); Label_5(input/queue); Label_6(render prep)
                           NETWORK_SET_THIS_SCRIPT_IS_NETWORK_SCRIPT; if pause return;
                           Switch Static[130] (48 states); Label_111 post-draw }
```
Selection via `Label_397` (X / R1+L1 combo), rendering via `DRAW_RECT/SPRITE/TEXT`, persistence via `Static[134]/136` single-slot queue → `Label_5` does `REQUEST_SCRIPT → HAS_SCRIPT_LOADED → START_NEW_SCRIPT`.
...
**Health check:** Works but shows its age — 64-char buffer overflow (L-01), single-slot queue race (L-02), no timeout (L-03), deprecated hash (L-04), `TERMINATE_ALL` overkill (L-05), PS3 dead code, no validation, no feedback — all fixed in `Label_1835_v2` (see `LOADER_DEEPDIVE_V2.md`). Built-ins duplicate outfit/teleport tables instead of data-driven.
...
| File | Lines | Role |
|---|---|---|
| `ModLoader.csa` | 29,938 | Text assembly source — *the mod* |
| `ModLoader_Statics.c` | 27 | Initial `Static_xxx = val;` — theme / flags (included at compile) |
| `ModLoader_vNext.csa` | 30,174 | vNext build with `Label_1835_v2` FIFO (docs) |
| `docs/CSA_BYTECODE_GUIDE.md` | 290 | VM opcode bible |
| `docs/DECOMPILED_WALKTHROUGH.md` | 291 | 4 hot functions annotated |
| `docs/LOADER_DEEPDIVE_V2.md` | 331 | Loader 6→8 param spec, FIFO, bugs |
| `docs/ModLoader_pseudo.c` | 8,771 | Auto pseudo-C (273 funcs) |
| `docs/callgraph.dot` | 3,868 edges | `Call @Label` graph |
| `tools/csa_decompile.py` | — | Disassembler / pseudo-C / stats |
| `tools/upgrade_loader.py` | — | L-01..L-11 checker + patcher |
| `tools/loader_v2_test.py` | — | FIFO/timeout sim |
...
- **Confirm:** `Label_397` checks `(Static[197]==Static[214] ? R1+L1 : X)`:

  ```asm
  StaticGet1 197 ; StaticGet1 214 ; JumpNE → check 177
  IS_DISABLED_CONTROL_PRESSED(202) && IS_DISABLED_CONTROL_PRESSED(203) OR IS_DISABLED_CONTROL_JUST_PRESSED(177) → PLAY_SOUND Click_Special → return 1
  ```
  `202/203` = R1/L1, `177` = X/A (`INPUT_FRONTEND_ACCEPT`)
...
| **13** | `Label_23` | `Label_74` | `Teleportations / Lugares Secretos` → Clothing/Ammu… | **Built-in teleport** | `SET_ENTITY_COORDS` via `Label_1239` |
...
| **34** | `Label_41` | `Label_106` | `Generador De Vehiculos / Spawn Vehicles` | **Vehicle built-in** | `CREATE_VEHICLE`, `SET_VEHICLE_MOD_KIT`, spawns via `Label_1509` hash switch (16 hashes) |
...
| **51** | `Label_53` | `Label_104` | `Opciones De Armas → Dar Todas Las Armas` | **Weapon built-in** | `GIVE_WEAPON_TO_PED`, `SET_PED_INFINITE_AMMO`, `GET_WEAPON_HUD_STATS` |
...
| `Static[130..142]` | **Menu state** `130`, timers `131`, queue `134/136`, pagination `174=page`, cursor `197`, `214` draw cursor, `224` X, FIFO `141/142` in vNext | |
...
| `Static2[434..442]` | **vNext FIFO** `queueScript[4]`, `queueStack[4]`, `queueVersion[4]` | |
...
|---|---|---|---|
| **Outfits** | 20,22-26,44,46-49 (8 states) | `SET_PED_COMPONENT_VARIATION` 12, `SET_PED_PROP_INDEX` 8, `CLEAR_PED_PROP` | Choose `SAAB Black Red` → `SET_PED_COMPONENT_VARIATION(ped, 3, 0, 0)` etc. 39 variants per menu, `SAAB Modz` credits |
| **Teleports** | 13-18 (6 states) | `SET_ENTITY_COORDS` 16, `GET_ENTITY_COORDS` | `Lugares Secretos → Military Base Tower` → `1239: get ped or vehicle, SET_ENTITY_COORDS(x,y,z)` |
| **Vehicles** | 34,52,54 (3 states) | `CREATE_VEHICLE` 5, `SET_VEHICLE_MOD` 38, `SET_VEHICLE_COLOURS` | `Generador → hash Switch Label_1375..1380` (switch on `Static[120..125]` picks `adder,zentorno,entityxf`) → `REQUEST_MODEL → CREATE_VEHICLE` |
...
| **Weapons** | 51 (state51) | `GIVE_WEAPON_TO_PED`, `GET_CURRENT_PED_WEAPON`, `SET_PED_INFINITE_AMMO_CLIP` | `Dar Todas Las Armas` loops `GIVE_DELAYED_WEAPON` 30× |
...
See `LOADER_DEEPDIVE_V2.md` L-01..L-11. Whole-mod extras:

| Area | Limitation | Impact |
|---|---|---|
| **Monolith** | Single 29,938-line file, 273 Functions interleaved — no `include` split | Merge conflicts, hard to theme |
...
| **Vehicle hash switch** | 6 switches `Label_1375..1380` with 10-18 hashes each, manual | Adding a car requires editing 6 places |
...
- **Find a vehicle spawn:** `grep -n "GET_HASH_KEY.*adder" ModLoader.csa` → hash switch at `Label_1375`
...
Now that you *understand* the whole mod (48 states, 9 loader dirs + 39 built-ins, 87 scripts, rendering/input/queue globals), next step is **plan the update**. See `docs/ROADMAP_vNext.md` for:

- Phase 1 Stabilize (FIFO, timeout, validation)
- Phase 2 UX (toasts, theme, bilingual search)
- Phase 3 Modularize (split monolith, data-driven outfits/teleports)
- Phase 4 Features (vehicle hash generator, favorites, updater)
- Timeline, risks, testing checklist

*This dossier is Step 1 — keep it as living doc; update after each phase.*
